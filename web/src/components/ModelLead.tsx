...
interface LeadModelData {
  auc: number;
  features: number;
  feature_importance: [string, number][];
  score_bands?: LeadBand[];
}

interface ScoreBandsResponse {
  bands: (LeadBand & { count: number; pct: number })[];
  total: number;
}

export default function ModelLead() {
  const [data, setData] = useState<LeadModelData | null>(null);
  const [bandsData, setBandsData] = useState<ScoreBandsResponse | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      apiFetch<LeadModelData>('/api/models/lead-scoring'),
      apiFetch<ScoreBandsResponse>('/api/score-bands/lead-scoring'),
    ])
  ...
  const impData = data.feature_importance.slice(0, 15).map(([n, v]) => ({
    name: n.replace(/_/g, ' '),
    ...
    <div className="space-y-8 animate-fade-in">
      <header>
        ...
        <p className="text-gray-500 mt-1">
          XGBoost + LightGBM ensemble for enrollment prediction
        </p>
      </header>
      ...
      {bandsData && (
        <section className="bg-slate-900/30 border border-white/5 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-4">
            Lead Score Bands & Interventions
          </h2>
          <p className="text-sm text-gray-500 mb-6">
            Leads segmented by enrollment likelihood. Each band maps to a specific outreach strategy.
          </p>
                ...
                <div className="p-4 space-y-3">
                  ...
                  {(b.count !== undefined || b.pct !== undefined) && (
                    <p className="text-sm font-mono pt-2 border-t border-white/10">
                      {b.count?.toLocaleString()} leads ({b.pct}%)
                    </p>
                  )}
                </div>
              </div>
